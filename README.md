...
Use the [webhook test endpoint](https://docs.arcadia.com/reference/test) to trigger a test webhook from Arcadia:
...
You should see a [`utility_credential_verified`](https://docs.arcadia.com/reference/utility-credential-verified) webhook event fired within seconds of submitting utility credentials. This webhook contains information about the submitted [UtilityCredential](https://docs.arcadia.com/reference/utility-credential-verified#utilitycredential). A [`utility_accounts_discovered`](https://docs.arcadia.com/reference/utility-accounts-discovered) webhook event should be fired shortly after. This webhook includes all the user's utility account data. Finally, after a minute or two, you'll receive a [`historical_utility_statements_discovered`](https://docs.arcadia.com/reference/historical-utility-statements-discovered) webhook event for each utility account discovered. These webhook events will contain information on existing utility statements for an account.
